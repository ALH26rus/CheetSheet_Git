...
`git diff` - посмотреть, что изменилось в коммите *(от англ. difference — «отличие», «разница»)*. По умолчанию команда `git diff` не показывает изменения в `staged`-файлах — только в `modified`.  
`git diff <hash> <hash>` - посмотреть, что изменилось в конкретных коммитах.  
`git diff --staged` - просмотреть изменения в `staged`.  
  
`echo "string" > <file>` - дописать новые строки, но перед этим стереть содержимое файла, то есть перезаписать файл целиком.  
`echo "string" >> <file>` - дописать новые строки в конец файла.  
  
### 3.1 Игнорирование файлов в Git  
Чтобы Git игнорировал какие-то файлы и не пытался добавить их в репозиторий, нужно создать файл **.gitignore** *(от англ. ignore — «игнорировать»)* и записать в него названия игнорируемых файлов.  
**.gitignore** - это обычный текстовый файл. Его добавляют в корень репозитория и тоже коммитят.  
 * Если строка начинается с `#`, то это комментарий, и **.gitignore** не будет его учитывать.  
 * Чтобы Git игнорировал все файлы, к примеру, `.DS_Store`. Для этого достаточно добавить в **.gitignore** строку с названием файла.  
 * Символ звёздочки `*` соответствует любой строке, включая пустую. Если такой символ используется в шаблоне в **.gitignore**, значит, файл будет проигнорирован вне зависимости от того, что будет на месте звёздочки. Пример:
```
...
`git status --ignored` - eсли нужно отобразить все игнорируемые файлы.  
